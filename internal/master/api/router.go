...
func registerAPI(r *gin.Engine, d *Deps) {
	...
	v1 := r.Group("/api/v1")
	{
		...
		)
		{
			...
			admin.GET("/update/releases", d.AdminUpdateReleases)
			...
			admin.GET("/servers/upgrade-status", d.AdminGetUpgradeStatuses)
			admin.POST("/servers/batch-upgrade", d.AdminBatchUpgradeServers)
			...
			admin.POST("/data/logs/cleanup", d.AdminCleanupLogs)
			admin.POST("/data/vacuum", d.AdminVacuum)
			admin.POST("/data/compact", d.AdminCompact)
