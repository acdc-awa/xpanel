...
// 主控无快照（如刚重启）才回退 servers.online_ips 列。
func TestAdminServerOnlineIPs_MemoryFirstAndOfflineZero(t *testing.T) {
	...
	fetch := func() (int, []struct {
		...
		c.Request = httptest.NewRequest(http.MethodGet, "/", nil)
		c.Params = gin.Params{{Key: "id", Value: strconv.FormatUint(srv.ID, 10)}}
		(&Deps{DB: db, Hub: h}).AdminServerOnlineIPs(c)
		if w.Code != http.StatusOK {
			t.Fatalf("status = %d, want 200", w.Code)
		}
		var resp struct {
	...
	// 在线：内存权威，DB 残影不漏出
	if n, users := fetch(); n != 1 || len(users) != 1 || users[0].Email != "u1.i1@panel.local" || users[0].IPs[0] != "1.1.1.1" {
		t.Fatalf("在线应读内存快照, got n=%d users=%+v", n, users)
	}

	// 离线：内存归零遮蔽 DB 残影
	h.SetOnlineForTest(srv.ID, time.Now().Add(-2*nodegate.HeartbeatTimeout))
	if n, users := fetch(); n != 0 || len(users) != 0 {
		t.Fatalf("离线应归零不回退 DB, got n=%d users=%+v", n, users)
	}

	// 主控无快照（重启后未收到心跳）：回退 DB 列
	h2 := nodegate.NewHub(db, nil, nil)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodGet, "/", nil)
	c.Params = gin.Params{{Key: "id", Value: strconv.FormatUint(srv.ID, 10)}}
	(&Deps{DB: db, Hub: h2}).AdminServerOnlineIPs(c)
	if w.Code != http.StatusOK {
		t.Fatalf("status = %d, want 200", w.Code)
	}
	var resp struct {
	...
	if resp.Data.OnlineUsers != 1 || len(resp.Data.Users) != 1 || resp.Data.Users[0].Email != "ghost@panel.local" {
		t.Fatalf("无快照应回退 DB 列, got n=%d users=%+v", resp.Data.OnlineUsers, resp.Data.Users)
	}
}
